 ...
 */
import { timeToSec, secToTime, nowUtcSec, toAbsSec } from '../utils/timeUtils.js';
...
export class TimeRibbon {
    ...
    _nowSec() { return toAbsSec(this._simTimeSec ?? nowUtcSec()) + this._viewOffsetSec; }
    ...
    _drawFlightBar(f, isBase = false, isWhatif = false) {
        ...
        // SIM 모드에서는 simTime >= depTime이면 가상 출발 처리
        const depSec = timeToSec(f.atd || f.ctot || f.eobt);
        const isDep = f.status === 'DEP' ||
            (this._simTimeSec !== null && this._simTimeSec >= depSec);
        ...
        if (isDep) barColor = 'rgba(52,199,89,0.2)';
        ...
        c.fillStyle = isDep ? '#4caf50' : (hasConflict ? '#ff6b6b' : airportText);
    ...
    _drawConvDiamonds() {
        ...
        flights.forEach(f => {
            ...
            wps.forEach(wp => {
                // 주 합류점과 RKPC 도착만 표시, 경유 웨이포인트는 제외
                if (wp.name !== convWp && wp.name !== 'RKPC') return;
