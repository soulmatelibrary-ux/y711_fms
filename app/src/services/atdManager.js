...
import { showToast, showUndoSimToast } from '../utils/toast.js';
...
export async function setAtd(flightId, atdHHMM, reason = 'manual') {
    ...
    // Undo 스택에 현재 상태 저장 (참조 보관하여 실패 시 정확한 항목 제거)
    const undoEntry = { flights: savedFlights };
    ...
    try {
        ...
        // 정확한 항목 찾아서 제거 (비동기 중 다른 변경이 있어도 안전)
        const idx = _undoStack.indexOf(undoEntry);
    ...
    showUndoSimToast(`ATD 설정 — ${flight.callsign}`, undoAtd, () => {
        document.dispatchEvent(new CustomEvent('sim:request', { detail: { flight } }));
    });
